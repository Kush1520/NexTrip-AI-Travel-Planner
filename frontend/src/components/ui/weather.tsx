...
const Weather = () => {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        // Use provided destination for fetching weather
        const apikey = import.meta.env.VITE_WEATHER_API_KEY;
        
        if (!apikey) {
          console.warn("Weather API key (VITE_WEATHER_API_KEY) is missing. Skipping weather fetch.");
          setError("API key missing" as any);
          setLoading(false);
          return;
        }
  ...
  if (error) {
    ...
    // OpenWeather API keys can take 10-30 minutes to activate after creation
    if (error.includes("401")) {
      return (
        <div className="p-4 border rounded-lg border-zinc-800 bg-black/60 text-yellow-400 text-center">
          <p className="text-sm">Predicting weather...</p>
          <p className="text-xs mt-1 text-zinc-500">Connecting to live weather stations...</p>
        </div>
      );
    }
