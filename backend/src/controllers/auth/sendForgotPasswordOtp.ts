...
export const sendForgotPasswordOtp = async (req: any, res: any) => {
  try {
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: 'Password Reset OTP - NexTrip',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
          <h2 style="color: #4a4a4a;">Reset Your Password</h2>
          <p>You requested a password reset for your NexTrip account. Use the following OTP to proceed:</p>
          ...
          <p style="margin-top: 30px; font-size: 12px; color: #777;">© ${new Date().getFullYear()} NexTrip. All rights reserved.</p>
        </div>
      `,
    });
